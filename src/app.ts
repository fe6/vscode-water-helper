...
export function encodeDocsUri(query?: Query): Uri {
  return Uri.parse(`water-helper://search?${JSON.stringify(query)}`);
}
...
export class App {
  ...
  getWebviewContent(query: Query) {
    const config = workspace.getConfiguration('water-helper');
...
export class WaterDocsContentProvider implements TextDocumentContentProvider {
...
export class WaterCompletionItemProvider implements CompletionItemProvider {
